import { Controller,UploadedFile, Get, Post, Put, Delete, Param, Body, Query, BadRequestException, UseGuards, UseInterceptors, ParseIntPipe } from '@nestjs/common';
...
async updateEmployee(@Param('id') id: string, @UploadedFile() file: Express.Multer.File, @Body() updates: Partial<Employee>): Promise<Employee | any> {
    const avatarPath = file ? `/uploads/${file.filename}` : null;
